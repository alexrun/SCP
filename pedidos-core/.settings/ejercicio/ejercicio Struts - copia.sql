...
create table jdbcdemo2.forma_pago(
  ...
  constraint fk_cat023 foreign key (item_ref) references jdbcdemo2.item(id) on delete restrict on update no action
...
create table jdbcdemo2.pago_cheque(
        ...
        fecha_cheque date,
        constraint fk_cat021 foreign key (id_pago_cheque) references jdbcdemo2.forma_pago(id) on delete restrict on update no action
);
...
create table jdbcdemo2.tarjeta_credito(
        ...
        banco varchar(20),
        constraint fk_cat021 foreign key (id_pago_tarjeta) references jdbcdemo2.forma_pago(id) on delete restrict on update no action
);
