...
create table jdbcdemo2.item(
        ...
        constraint fk_cat022 foreign key (categoria_ref) references jdbcdemo2.categoria(id) on delete restrict on update no action
);
...
create table jdbcdemo2.pedidos(
        ...
        estatus varchar(10),
        constraint fk_cat021 foreign key (cliente_ref) references jdbcdemo2.cliente2(id) on delete restrict on update no action
...
create table jdbcdemo2.detalle_pedido(
        ...
        constraint fk_cat661 foreign key (pedido_ref) references jdbcdemo2.pedidos(id) on delete restrict on update no action,
        constraint fk_cat041 foreign key (item_ref) references jdbcdemo2.item(id) on delete restrict on update no action
...
create table jdbcdemo2.forma_pago(
  ...
  constraint fk_cat023 foreign key (pedido_ref) references jdbcdemo2.pedidos(id) on delete restrict on update no action
...
create table jdbcdemo2.pago_cheque(
        ...
        fecha_cheque date,
        constraint fk_cat077 foreign key (id_pago_cheque) references jdbcdemo2.forma_pago(id) on delete restrict on update no action
);
...
create table jdbcdemo2.tarjeta_credito(
        ...
        banco varchar(20),
        constraint fk_cat087 foreign key (id_pago_tarjeta) references jdbcdemo2.forma_pago(id) on delete restrict on update no action
);
